...
public:
	...
	void describe();
	void produceStock(Stock *stock, int currentCycle, std::string processName = "", std::map<std::string, Process *> processMap = std::map<std::string, Process *>());
	bool selectProcess(Stock *stock, std::string &process);
	bool canBeProduce(Process *process);
	void productStockProcess(Process *process, int currentCycle, std::string processName);
